...
/// change, invitation removal, place creation, place edit, or rating —
/// merged from several tables and sorted newest first. `summary` and
/// `actor` are pre-formatted server-side since the underlying sources have
/// very different shapes.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct ActivityEntry {
    /// "login" | "failed_login" | "profile_change" | "invite_change" | "place_created" | "place_change" | "rating"
    pub kind: String,
    pub summary: String,
    pub actor: String,
    pub created_at: String,
