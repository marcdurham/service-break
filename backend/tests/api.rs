...
use serde_json::json;
use shared::{
    AuthSession, ImportSummary, Invitation, Parking, PlaceDetail, PlaceSummary, PlaceType,
    Requirement,
};
use sqlx::PgPool;
use uuid::Uuid;
...
async fn admin_export_import_round_trip(pool: PgPool) {
    ...
    let req = TestRequest::put()
        ...
        .to_request();
    assert_eq!(call_service(&app, req).await.status(), StatusCode::NO_CONTENT);
    let req = TestRequest::post().uri("/api/invites").insert_header(auth(&scout)).to_request();
    assert_eq!(call_service(&app, req).await.status(), StatusCode::CREATED);
