...
- 16:45 — Made `start.sh` generate a temporary Trunk.toml that proxies `/api` to the backend on the same `8000+SUFFIX` port, so API calls work regardless of which suffix is used.
