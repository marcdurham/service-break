...
- 14:15 — Added structured logging shipped to [OpenObserve](https://openobserve.ai/): a new `openobserve` container in `docker-compose.yml` (not profile-gated, so it starts with plain `docker compose up -d`), a background JSON-batch shipper (`backend/src/telemetry.rs`, enabled only when `OPENOBSERVE_URL` is set — stdout logging is unaffected either way), and `tracing-actix-web`'s `TracingLogger` middleware for a log line on every REST request (method, route, status, latency). Added explicit log events for login/failed-login (all three failure reasons), registration, Google sign-in/link/unlink, place create/update/delete, and tile cache hit/miss/eviction/errors. See the new "Structured logging (OpenObserve)" section in `DEPLOY.md`.
- 09:22 — Tapping Edit on an unvisited (Overpass) place while signed out now takes the user to the sign-in page instead of just flashing a "Sign in to save places" toast, and brings them back to editing that exact place after they sign in. New URL route `/poi/:kind/:num/edit` (`Route::PoiEdit`) carries the intent: signed out it renders the sign-in page (password sign-in resumes in place; the Google OAuth round trip returns via a `sessionStorage` path that `/oauth-complete` honors for this route), signed in it promotes the POI and opens the place with the edit form already open (new one-shot `start_editing` prop on `DetailView`). Save/Rate while signed out also now actually show the Account page — the POI overlay used to stay on top of it.
- 01:56 — Tapping an unvisited (blue/gray Overpass) map pin now shows the same compact bottom tile app places get — badge, name, type, distance, and a "Directions to …" button — with the pin highlighted, instead of jumping straight to the full-page preview; tapping the tile opens the full preview (`PoiDetailView`), and empty-map taps, closing, selecting an app place, or turning discovery off all dismiss it.
- 01:55 — Halved the gap between the featured card's Directions button and the tab bar (card anchored at 86px instead of 100px above the bottom; the Discover/recenter buttons and the discover tip shift down to match).
- 01:43 — Moved the OSM map-tile cache from Postgres to a disk-based LRU cache (new `backend/src/tile_cache.rs`): tiles now live as `{z}/{x}/{y}.png` files under `TILE_CACHE_DIR` (default `./tile-cache`, gitignored) with file mtime as the fetch time for the same 30-day TTL and stale-if-upstream-down behavior, capped at `TILE_CACHE_MAX_MB` (default 512) with least-recently-used tiles evicted from disk when over budget; the index is rebuilt by rescanning the directory on startup. The `map_tiles` table is dropped (new migration), and docker-compose mounts a `tile-cache` volume so the cache persists across deploys.
...
- 21:30 — Added an account activity log: the last 50 logins, failed logins, profile changes (username/password/admin flag/name — by the account or an admin), place edits, and ratings for a user. Admins reach it via a "View activity" button on the Edit user page (`/users/:id/activity`); users reach their own via "My activity" on the Account page (`/activity`). New `user_activity` table plus `GET /api/auth/activity` and `GET /api/admin/users/{id}/activity` endpoints; place edits and reviews are merged in from their existing tables rather than duplicated.
